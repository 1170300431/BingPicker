...
<<<<<<< HEAD:GUI.h
	int create() {
		...
		if (!hwnd)
		{
			char s[10];
			_itoa_s(GetLastError(), s, 10);
			MessageBox(NULL, TEXT("¥¥Ω®¥∞ø⁄ ß∞‹£°"), s, MB_OK);
	...
	unsigned long long show() {				//‘⁄¥À¥¶÷˜≥Ã–Úπ“∆
		MSG msg;
...
unsigned long long form::show() {				//Âú®Ê≠§Â§Ñ‰∏ªÁ®ãÂ∫èÊåÇËµ∑
	MSG msg;
	ShowWindow(hwnd, SW_SHOW); UpdateWindow(hwnd);
	ZeroMemory(&msg, sizeof(msg));
	if (this->Event_Load_Complete) this->Event_Load_Complete(this);
...
public:
	...
	int create() {
		...
		if (!hWnd) {
			char s[10];
			_itoa_s(GetLastError(), s, 10);
			MessageBox(NULL, TEXT("¥¥Ω®∞¥≈• ß∞‹£°"), s, MB_OK);
			return -1;
		}
		return id;
	}
...
public:
	...
	int create() {
		...
		if (!hWnd) {
			char s[10];
			_itoa_s(GetLastError(), s, 10);
			MessageBox(NULL, TEXT("¥¥Ω®Œƒ±æøÚ ß∞‹£°"), s, MB_OK);
			return -1;
		}
		return id;
	}
...
public:
	...
	int create() {
		...
		if (!hWnd) {
			char s[10];
			_itoa_s(GetLastError(), s, 10);
			MessageBox(NULL, TEXT("¥¥Ω®±Í«©øÚ ß∞‹£°"), s, MB_OK);
			return -1;
		}
		return id;
	}
	...
	int create() {
		...
		if (hWnd) {
			...
			if (!hbmp) {
				char s[10];
				_itoa_s(GetLastError(), s, 10);
				MessageBox(NULL, TEXT("∂¡»°ŒªÕº ß∞‹£°"), s, MB_OK);
				return -1;
			}
...
public:
	...
	int create() {
		...
		if (!hWnd) {
			char s[10];
			_itoa_s(GetLastError(), s, 10);
			MessageBox(NULL, TEXT("¥¥Ω®Ω¯∂»Ãı ß∞‹£°"), s, MB_OK);
			return -1;
		}
		return id;
	}
